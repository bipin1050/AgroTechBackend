...
module.exports.createReview = async function createReview(req, res) {
  try {
    ...
    let checkDuplicate = await reviewModel.find({
      plan: id,
      user: req.id,
    });
    if (checkDuplicate.length > 0) {
      return res.status(400).json({
        message: "You've already reviwed/rated",
      });
    }
    ...
    plan.ratingsAverage =
      (plan.ratingsAverage * (count - 1) + parseInt(req.body.rating)) / count;
    await plan.save();
    res.json({
...
module.exports.getBlogs = async (req, res) => {
  try {
    ...
    let blogs = await blogModel
      .find()
      .select("title author publicationDate highlights image");
    res.status(200).json({
